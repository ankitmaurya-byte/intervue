...
import { banTabId } from '../utils/banUtils';
...
const ChatWidget = ({ userType, studentId, name }) => {
  ...
  useEffect(() => {
    ...
    const onJoined = ({ userType, name, id, students }) => {
      ...
      setMessages(m => [...m, { system: true, text: `${name} joined as ${userType}`, ts: Date.now() }]);
    };
    ...
    const onKicked = ({ studentId, name }) => {
      setMessages(m => [...m, { system: true, text: `${name} was removed by the teacher`, ts: Date.now() }]);
    ...
    const onForce = ({ reason }) => {
      ...
      setMessages(m => [...m, { system: true, text: reason || 'Removed by the teacher', ts: Date.now() }]);
...
socket.on('kicked', () => {
  ...
  const sendMsg = () => {
    ...
    if (!text) return;
    const payload = { userId: studentId || 'teacher', userType, name: name || 'Teacher', text };
  ...
  return (
    <>
      {/* Floating button */}
      <button className="td-chat" aria-label="Open help chat" onClick={() => setOpen(v => !v)}>
        💬
      </button>
      ...
      {open && (
        <div className="cw-panel" role="dialog" aria-label="Class chat">
          <div className="cw-tabs">
            <button className={`cw-tab ${tab === 'chat' ? 'active' : ''}`} onClick={() => setTab('chat')}>Chat</button>
            <button className={`cw-tab ${tab === 'participants' ? 'active' : ''}`} onClick={() => setTab('participants')}>Participants</button>
          </div>
          ...
          {tab === 'chat' ? (
            <div className="cw-body" ref={listRef}>
              {messages.map((m, i) =>
                m.system ? (
                  <div key={i} className="cw-system">{m.text}</div>
                ) : (
                  <div key={i} className={`cw-msg ${m.userId === studentId && userType !== 'teacher' ? 'mine' : (userType === 'teacher' && m.userType === 'teacher' ? 'mine' : '')}`}>
                    <div className={`cw-msg-user ${m.userType === 'teacher' ? 'teacher' : ''}`}>
                      {m.userType === 'teacher' ? 'Teacher' : m.name || 'User'}
                    </div>
                    <div className={`cw-bubble ${m.userType === 'teacher' ? 'teacher' : 'student'}`}>
                      {m.text}
                    </div>
                  </div>
                )
            ...
            <div className="cw-body">
              ...
              {students.length === 0 && <div className="cw-empty">No students yet</div>}

              {students.map(s => (
                <div key={s.studentId} className="cw-part-row">
                  <span className="cw-name">{s.name}</span>
                  {userType === 'teacher' && (
                    <button className="cw-kick" onClick={() => handleKick(s.studentId)}>Kick out</button>
                  )}
                </div>
              ))}
            </div>
